      ...
      void (async () => {
        try {
          ...
          const filesText = files.length > 0 
            ? files.map(f => `- [${f.name}](file://${f.path}) (${f.type})`).join("\n")
            : "No files detected yet.";
            
          const acodeMdContent = `# ACode Workspace Overview: ${workspace.name}

## Project Summary
Auto-generated by ACode \`/init\` command on ${new Date().toLocaleDateString()}.

## Directory Layout
${filesText}

## Workspace Instructions
You can modify this file to document build commands, project rules, and file layout instructions for the AI assistant.
`;
            ...
            {gitStatus && (totalAdded + totalDeleted + totalModified) > 0 && (
              <div className="flex items-center gap-2 mb-4 text-[11px] text-acode-text-muted">
                ...
                <span className="ml-auto flex items-center gap-1">
                  ...
                  {currentModel?.model.name || selectedModelId || ""}
                </span>
              </div>
            )}
            {hasMessages && (
