 ...
 * ACode's terminal-style chat input:
 *
...
import type { FileNode, ChatSessionSummary } from "@acode/shared-types";
...
const SLASH_COMMANDS: SlashCommand[] = [
  { id: "init",      label: "/init",      description: "Scan the workspace and bootstrap ACODE.md",  insert: "/init " },
 ...
 * menu — that's the same rule ACode's terminal uses.
 */
...
export function PromptAutocomplete({
  ...
  return (
    <div
      className="absolute z-50 left-0 right-0 bottom-full mb-1.5 mx-3 bg-acode-bg-secondary border border-acode-border-primary rounded-xl shadow-2xl overflow-hidden"
      onMouseDown={(e) => e.preventDefault()}
    >
      <div className="px-3 py-1.5 flex items-center gap-2 text-[10px] uppercase tracking-wider text-acode-text-muted border-b border-acode-border-primary">
        ...
        <span className="ml-auto flex items-center gap-2 normal-case tracking-normal text-acode-text-muted">
          ...
          <li key={optionKey(opt, idx)}>
            <button
              ...
              className={`w-full text-left px-3 py-1.5 flex items-center gap-2 ${
                idx === activeIdx ? "bg-acode-bg-hover" : "hover:bg-acode-bg-hover/50"
              }`}
            >
              {renderOption(opt)}
            </button>
...
function renderOption(opt: Option) {
  if (opt.kind === "command") {
    return (
      <>
        <Terminal className="w-3.5 h-3.5 text-acode-text-muted flex-shrink-0" />
        <span className="text-sm text-acode-text-primary font-mono">{opt.label}</span>
        <span className="text-xs text-acode-text-muted truncate ml-2">{opt.description}</span>
      </>
    );
  }
  if (opt.kind === "file") {
    return (
      <>
        <FileText className="w-3.5 h-3.5 text-acode-text-muted flex-shrink-0" />
        <span className="text-sm text-acode-text-primary truncate">{opt.name}</span>
        <span className="text-[11px] text-acode-text-muted font-mono truncate ml-2">{opt.path}</span>
      </>
    );
  }
  if (opt.kind === "skill") {
    return (
      <>
        <Sparkles className="w-3.5 h-3.5 text-acode-accent-primary flex-shrink-0" />
        <span className="text-sm text-acode-text-primary font-mono">${opt.name}</span>
        <span className="text-xs text-acode-text-muted truncate ml-2">{opt.description}</span>
      </>
    );
  }
  return (
    <>
      <Hash className="w-3.5 h-3.5 text-acode-text-muted flex-shrink-0" />
      <span className="text-sm text-acode-text-primary truncate">{opt.title}</span>
      <span className="text-[10px] text-acode-text-muted ml-2 flex-shrink-0">{opt.status}</span>
    </>
  );
}
