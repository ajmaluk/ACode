...
import { useUI, useChat, useWorkspace, useSettingsView, useSettings, useTerminal, useCommandPalette, useShortcuts } from "@/store/useAppStore";
...
function useEditorMenus(): { label: string; items: MenuAction[] }[] {
  ...
  const saveFile = async () => {
    ...
    try {
      ...
      toast({ kind: "error", title: "Save failed", description: (err as Error)?.message ?? "Unknown error" });
    }
  };
  ...
  const saveAllFiles = async () => {
    ...
    try {
      ...
      toast({ kind: "success", title: "All files saved", description: `${dirty.length} file(s)` });
    } catch (err) {
      toast({ kind: "error", title: "Save all failed", description: (err as Error)?.message ?? "Unknown error" });
    }
  };
  ...
  return [
    {
      label: "File",
      items: [
        { type: "item", label: "New File", shortcut: shortcut("N"), perform: () => { if (ws) void openFile(ws.path + "/untitled"); } },
        { type: "item", label: "Open File…", shortcut: shortcut("O"), perform: () => void openWorkspace() },
        { type: "separator" },
        { type: "item", label: "Save", shortcut: shortcut("S"), perform: () => void saveFile() },
        { type: "item", label: "Save All", shortcut: shortcut("S", { shift: true }), perform: () => void saveAllFiles() },
        { type: "separator" },
        { type: "item", label: "Close Tab", shortcut: shortcut("W"), perform: () => { const { activeFilePath } = useWorkspace.getState(); if (activeFilePath) closeTab(activeFilePath); } },
        { type: "item", label: "Close All", shortcut: `${modKey()} K W`, perform: closeAllTabs },
        { type: "item", label: "Close Others", perform: closeOtherTabs },
        { type: "separator" },
        { type: "item", label: "Preferences", shortcut: `${modKey()},`, perform: () => openSettings() },
      ],
    },
    {
      label: "Edit",
      items: [
        ...
        { type: "separator" },
        { type: "item", label: "Toggle Comment", shortcut: `${modKey()}/`, perform: () => window.dispatchEvent(new CustomEvent("editor:toggle-comment")) },
      ],
    },
    {
      label: "View",
      items: [
        ...
        { type: "separator" },
        { type: "item", label: "Zoom In", shortcut: `${modKey()}+`, perform: () => { void updateSetting("codeFontSize", Math.min((settings.codeFontSize ?? 14) + 1, 32)); } },
        { type: "item", label: "Zoom Out", shortcut: `${modKey()}-`, perform: () => { void updateSetting("codeFontSize", Math.max((settings.codeFontSize ?? 14) - 1, 8)); } },
        { type: "separator" },
        { type: "item", label: "Agent Mode", shortcut: shortcut("E"), perform: () => setViewMode("chat") },
        { type: "item", label: "Editor Mode", shortcut: shortcut("E", { shift: true }), perform: () => setViewMode("editor") },
      ],
    },
    {
      label: "Go",
      items: [
        { type: "item", label: "Quick Open", shortcut: shortcut("P"), perform: () => window.dispatchEvent(new CustomEvent("editor:quick-open")) },
        { type: "item", label: "Go to Line…", shortcut: shortcut("G"), perform: () => window.dispatchEvent(new CustomEvent("editor:go-to-line")) },
        { type: "separator" },
        { type: "item", label: "Back", shortcut: `${modKey()}[`, perform: () => useChat.getState().goBackChat() },
        { type: "item", label: "Forward", shortcut: `${modKey()}]`, perform: () => useChat.getState().goForwardChat() },
      ],
    },
    {
      label: "Terminal",
      items: [
        { type: "item", label: "New Terminal", shortcut: `${modKey()}\``, perform: toggleTerminal },
        { type: "item", label: "Toggle Terminal", shortcut: shortcut("J"), perform: toggleTerminal },
      ],
    },
    {
      label: "Help",
      items: [
        { type: "item", label: "Keyboard Shortcuts", shortcut: "?", perform: () => useShortcuts.getState().toggle() },
        { type: "item", label: "Command Palette", shortcut: shortcut("K"), perform: () => useCommandPalette.getState().toggle() },
        { type: "separator" },
        { type: "item", label: "Open Settings", shortcut: `${modKey()},`, perform: () => openSettings() },
      ],
    },
  ];
}
...
export function TitleBar() {
  ...
  const [detectedIdes, setDetectedIdes] = useState<{ name: string; command: string; kind: string }[]>([]);
  ...
  const openInApp = async (app: string) => {
    ...
    try {
      ...
      if (app === "finder") {
        ...
        toast({ kind: "success", title: `Opened in ${label}`, description: path });
      }
    } catch (err) {
      toast({ kind: "error", title: `Failed to open`, description: (err as Error)?.message ?? String(err) });
    }
  };
  ...
  return (
    ...
    >
            ...
            <div key={m.label} className="relative">
              <button
                className={`px-2.5 h-7 text-[11px] text-dalam-text-secondary hover:text-dalam-text-primary hover:bg-dalam-bg-hover transition-colors ${
                  openIdx === i ? "bg-dalam-bg-hover text-dalam-text-primary" : ""
                }`}
            ...
            <div className="absolute left-1/2 -translate-x-1/2 top-full mt-1 w-64 bg-dalam-bg-secondary border border-dalam-border-primary rounded-lg shadow-2xl z-50 overflow-hidden" role="menu">
              ...
              <button onClick={() => openInApp("terminal")} role="menuitem"
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs text-dalam-text-primary font-medium">Terminal</div>
              ...
              <button onClick={() => openInApp("finder")} role="menuitem"
                ...
                <div className="flex-1 min-w-0">
                  <div className="text-xs text-dalam-text-primary font-medium">File manager</div>
                  <div className="text-[10px] text-dalam-text-muted truncate">Reveal in file manager</div>
                </div>
              </button>
                ...
                <div className="border-t border-dalam-border-primary">
                  ...
                  {detectedIdes.map((ide) => (
                    <button key={ide.command} onClick={() => openInApp(ide.command)} role="menuitem"
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="text-xs text-dalam-text-primary font-medium">{ide.name}</div>
                        <div className="text-[10px] text-dalam-text-muted truncate">{ide.command}</div>
                      </div>
                    </button>
                  ))}
                </div>
      ...
      {memoryActive && (
        <span title="Workspace Memory Active" className="flex items-center flex-shrink-0 ml-1">
          <Brain className="w-3.5 h-3.5 text-dalam-accent-primary animate-pulse-soft" />
        </span>
      )}

      {isCompacted && (
        <span className="flex items-center gap-1.5 px-2 py-0.5 text-[9px] font-semibold bg-dalam-accent-subtle text-dalam-accent-primary rounded-full uppercase tracking-wider animate-pulse-soft flex-shrink-0 ml-1" title="Compacted">
          Compacted
        </span>
      )}
      ...
      <div className="flex items-center gap-0.5 px-1.5">
        ...
        <div className={`relative flex items-center bg-dalam-bg-tertiary rounded-full border border-dalam-border-primary p-0.5 ${!activeWorkspaceId ? "opacity-40" : ""}`}>
          <button
            ...
            title={!activeWorkspaceId ? "Open a folder to switch modes" : "Agent mode (Ctrl+E)"}
          ...
          <button
            ...
            title={!activeWorkspaceId ? "Open a folder to switch modes" : "Editor mode (Ctrl+E)"}
            disabled={!activeWorkspaceId}
          >
        ...
        <Tooltip content={`Settings (${mod},)`} side="bottom">
          <button className="w-7 h-7 flex items-center justify-center rounded-md text-dalam-text-secondary hover:bg-dalam-bg-hover transition-colors"
...
function MenuPanel({ items, onClose }: { items: MenuAction[]; onClose: () => void }) {
  return (
    <div
        ...
        if (action.type === "item") {
          return (
            ...
            >
              ...
              {action.shortcut && (
                <kbd className="text-[10px] text-dalam-text-muted whitespace-nowrap">{action.shortcut}</kbd>
              )}
            </button>
          );
        }
