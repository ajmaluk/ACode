...
// ============================================================================
export function VersionRestoreBar({
  restoredVersionId,
  activeSessionId,
  sessionVersions,
  onConfirm,
    ...
    <div className="px-3 pt-1.5 pb-0 flex-shrink-0 bg-dalam-bg-primary">
      <div className="flex items-center gap-2 px-3 py-1.5 bg-dalam-accent-subtle/40 border border-dalam-accent-primary/20 rounded-lg text-xs">
        ...
        <div className="flex-1 min-w-0">
          <span className="text-dalam-text-primary font-medium truncate">
            {ver.label}
          </span>
          <span className="text-dalam-text-muted ml-1.5">
            · {ver.messages.length} message
            {ver.messages.length !== 1 ? "s" : ""}
          </span>
        </div>
        <button
          type="button"
...
export function ResetConfirmDialog({
  ...
  return (
    ...
    >
      <div
        className="w-[440px] max-h-[70vh] bg-dalam-bg-primary border border-dalam-border-primary rounded-xl shadow-2xl flex flex-col overflow-hidden animate-fade-in"
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin p-3">
          {loading ? (
            ...
            <div className="text-center text-xs text-dalam-text-muted py-8">
              No file changes will be reverted.
            </div>
          ) : (
            <div className="space-y-1">
              ...
              {fileChanges.map((fc) => (
                <div
                  key={fc.path}
                  className="border border-dalam-border-primary/50 rounded-lg overflow-hidden"
                >
                  <button
                    type="button"
                  ...
                  >
                    <ChevronDown
                      className={`w-3 h-3 text-dalam-text-muted transition-transform flex-shrink-0 ${expandedFiles.has(fc.path) ? "" : "-rotate-90"}`}
                    />
                    <FileCode className="w-3 h-3 text-dalam-text-muted flex-shrink-0" />
                    <span className="flex-1 min-w-0 text-xs text-dalam-text-primary truncate font-mono">
                      {fc.path}
                    </span>
                    {fc.additions > 0 && (
                      <span className="text-[10px] text-dalam-git-added font-mono">
                        +{fc.additions}
                      </span>
                    )}
                    {fc.deletions > 0 && (
                      <span className="text-[10px] text-dalam-git-deleted font-mono">
                        -{fc.deletions}
                      </span>
                    )}
                  </button>
...
export function RestorePopup({
  ...
  const assistantMsgCount = removedMessages.filter(
    (m) => m.role === "assistant",
  ).length;
  return (
    <div className="mb-2 px-3">
      <div className="max-w-2xl mx-auto bg-dalam-bg-secondary border border-dalam-accent-primary/30 rounded-lg shadow-lg px-3 py-2 flex items-center gap-2 animate-fade-in">
        ...
        <div className="flex-1 min-w-0 text-[11px] text-dalam-text-secondary">
          <span className="text-dalam-text-primary font-medium">
            {removedMessages.length} message
            {removedMessages.length !== 1 ? "s" : ""}
          </span>{" "}
