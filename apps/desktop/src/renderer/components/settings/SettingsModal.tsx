...
export function SettingsModal() {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-acode-bg-primary flex flex-col animate-fade-in">
      <div className="flex flex-1 min-h-0">
        <nav className="w-56 flex-shrink-0 border-r border-acode-border-primary bg-acode-bg-secondary py-4 flex flex-col">
          <button className="mx-4 mb-4 flex items-center gap-2 text-sm text-acode-text-secondary hover:text-acode-text-primary transition-colors" onClick={close}>
            ...
            {TABS.map((t) => {
              ...
              return (
                <button key={t.id} onClick={() => setActiveTab(t.id)}
                  className={`w-full text-left flex items-center gap-2.5 px-3 py-2 text-sm rounded-lg transition-colors mb-0.5 ${active ? "bg-acode-accent-subtle text-acode-text-primary" : "text-acode-text-secondary hover:bg-acode-bg-hover hover:text-acode-text-primary"}`}>
...
function Card({ title, description, children }: { title: string; description?: string; children: React.ReactNode }) {
  return (
    <div className="bg-acode-bg-secondary border border-acode-border-primary rounded-xl p-6 mb-4">
      <div className="mb-4"><h3 className="text-sm font-medium text-acode-text-primary">{title}</h3>{description && <p className="text-xs text-acode-text-muted mt-1">{description}</p>}</div>
      {children}
    </div>
  );
}
...
function Section({ title, hint, children }: { title: string; hint?: string; children?: React.ReactNode }) {
  return (
    <section className="mb-8">
      <h3 className="text-xs uppercase tracking-wider text-acode-text-muted mb-1">{title}</h3>
      {hint && <p className="text-xs text-acode-text-muted mb-3">{hint}</p>}
      {children}
    </section>
  );
}
...
function GeneralTab() {
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-acode-text-primary mb-8">General</h1>
        ...
        {(["Dark", "Light", "System default"] as const).map((label) => {
          ...
          return (
            <button key={label} onClick={() => update("theme", val)}
              className={`px-4 py-1.5 rounded-lg text-sm border transition-colors ${isActive ? "bg-acode-bg-active text-acode-text-primary border-acode-border-primary" : "bg-acode-bg-secondary text-acode-text-secondary border-acode-border-primary hover:bg-acode-bg-hover"}`}>
              {label}
            </button>
          );
        })}
      ...
      <Card title="Language" description="Choose the display language used by the application UI.">
        <div className="flex justify-end">
          <div className="relative">
            ...
            <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-acode-text-muted pointer-events-none" />
          </div>
        </div>
      </Card>
      <Card title="Interface zoom" description="Adjust the overall size of text and controls in the current window.">
        <div className="flex justify-end">
          <div className="flex rounded-lg border border-acode-border-primary overflow-hidden">
            {(["Smaller", "Default", "Larger"] as const).map((label) => {
              ...
              return (
                <button key={label} className={`px-4 py-1.5 text-sm transition-colors ${isActive ? "bg-acode-bg-active text-acode-text-primary" : "bg-acode-bg-secondary text-acode-text-secondary hover:bg-acode-bg-hover"}`}
      ...
      <div className="my-6 border-t border-acode-border-primary" />
      ...
      <Card title="Terminal font" description="Leave blank to auto-detect system terminal settings; set a value to override the ACode terminal font.">
        <div className="flex items-center gap-3">
          ...
          <button className="px-4 py-1.5 bg-acode-bg-active hover:bg-acode-bg-tertiary text-sm text-acode-text-primary rounded-md border border-acode-border-primary transition-colors" onClick={() => update("terminalFont", terminalFont)}>Save</button>
        </div>
      </Card>
      <div className="my-6 border-t border-acode-border-primary" />
      <Card title="HTTP Proxy" description="Route model, MCP, command-tool, and app renderer egress traffic through this proxy. Leave blank for direct connections. Restart the app to take effect.">
        <div className="flex items-center gap-3">
          ...
          <button className="px-4 py-1.5 bg-acode-bg-active hover:bg-acode-bg-tertiary text-sm text-acode-text-primary rounded-md border border-acode-border-primary transition-colors" onClick={() => update("httpProxy", httpProxy)}>Save</button>
        </div>
      </Card>
    </>
  );
...
function CodePreviewTab() {
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-acode-text-primary mb-8">Code preview</h1>
      <Card title="Light code theme" description="Theme used for code blocks while the interface is in light mode.">
        <div className="flex justify-end"><div className="relative"><select className="input-base w-56 appearance-none pr-8" value={settings.codeThemeLight} onChange={(e) => update("codeThemeLight", e.target.value)}><option value="github-light">GitHub Light</option><option value="solarized-light">Solarized Light</option></select><ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-acode-text-muted pointer-events-none" /></div></div>
      </Card>
      <Card title="Dark code theme" description="Theme used for code blocks while the interface is in dark mode.">
        <div className="flex justify-end"><div className="relative"><select className="input-base w-56 appearance-none pr-8" value={settings.codeThemeDark} onChange={(e) => update("codeThemeDark", e.target.value)}><option value="acode-dark">ACode Dark</option><option value="github-dark">GitHub Dark</option><option value="one-dark">One Dark</option><option value="dracula">Dracula</option></select><ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-acode-text-muted pointer-events-none" /></div></div>
      </Card>
      ...
      <Card title="Code font size" description="Adjust the default font size used by code previews.">
        <div className="flex items-center gap-4">
          <input type="range" min={10} max={22} value={settings.codeFontSize} onChange={(e) => update("codeFontSize", Number(e.target.value))} className="flex-1 accent-acode-accent-primary" />
          <span className="text-sm text-acode-text-primary w-8 text-right">{settings.codeFontSize}</span>
        </div>
      </Card>
      <div className="mt-8">
        <h3 className="text-sm font-medium text-acode-text-primary mb-2">Live preview</h3>
        <p className="text-xs text-acode-text-muted mb-4">The code viewer on the right automatically switches to the matching theme for the current app mode.</p>
        <div className="grid grid-cols-2 gap-4">
          {(["Light", "Dark"] as const).map((mode) => (
            <div key={mode} className="bg-acode-bg-secondary border border-acode-border-primary rounded-xl p-4">
              <div className="flex items-center justify-between mb-3">
                <div>
                  <div className="text-sm font-medium text-acode-text-primary">{mode} preview</div>
                  <div className="text-xs text-acode-text-muted">{mode === "Light" ? settings.codeThemeLight : settings.codeThemeDark}</div>
                </div>
                <span className={`px-2 py-0.5 text-[10px] rounded ${mode === "Dark" ? "bg-acode-accent-primary text-white" : "bg-acode-bg-tertiary text-acode-text-secondary border border-acode-border-primary"}`}>{mode === "Dark" ? "Active" : mode}</span>
              </div>
              <pre className="text-mono text-xs leading-relaxed text-acode-text-primary bg-acode-bg-primary rounded-lg p-3 overflow-x-auto"><code>{`const themePreview: ThemeConfig = {\n  surface: "sidebar",\n  accent: "#339CFF",\n  contrast: 45,\n};`}</code></pre>
            </div>
          ))}
        </div>
      </div>
...
function ModelsTab() {
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-acode-text-primary mb-2">Model settings</h1>
      <p className="text-sm text-acode-text-muted mb-8">Manage custom model providers. Once configured, they can be selected during chat.</p>
      <div className="flex gap-0 border border-acode-border-primary rounded-xl overflow-hidden bg-acode-bg-secondary">
        <div className="w-56 flex-shrink-0 border-r border-acode-border-primary">
          <div className="px-4 py-3 text-xs text-acode-text-muted uppercase tracking-wider border-b border-acode-border-primary">Providers</div>
          <div className="p-2">
            {providers.filter(p => p.type === "built-in").map((p) => (
              <button key={p.id} className={`w-full text-left flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors mb-0.5 ${selectedProviderId === p.id ? "bg-acode-accent-subtle text-acode-text-primary" : "text-acode-text-secondary hover:bg-acode-bg-hover"}`}
                onClick={() => { setSelectedProvider(p.id); setShowAdd(false); }}>
                <span className="w-5 h-5 rounded bg-acode-bg-active flex items-center justify-center text-[10px] font-bold text-acode-text-primary">A</span>
                <span className="flex-1 truncate">{p.name}</span>
                <span className={`w-2 h-2 rounded-full ${p.apiKey ? "bg-acode-git-added" : "bg-acode-text-muted"}`} />
              </button>
            ))}
          </div>
          <div className="px-4 py-2 text-xs text-acode-text-muted uppercase tracking-wider border-t border-b border-acode-border-primary">Custom providers</div>
          <div className="p-2">
            {providers.filter(p => p.type === "custom").map((p) => (
              <button key={p.id} className={`w-full text-left flex items-center gap-2 px-3 py-2 rounded-lg text-sm transition-colors mb-0.5 ${selectedProviderId === p.id ? "bg-acode-accent-subtle text-acode-text-primary" : "text-acode-text-secondary hover:bg-acode-bg-hover"}`}
                onClick={() => { setSelectedProvider(p.id); setShowAdd(false); }}>
                <span className="w-5 h-5 rounded bg-acode-bg-active flex items-center justify-center text-[10px] font-bold text-acode-text-primary">{p.name[0]}</span>
                <span className="flex-1 truncate">{p.name}</span>
                <span className={`w-2 h-2 rounded-full ${p.apiKey ? "bg-acode-git-added" : "bg-acode-text-muted"}`} />
              </button>
            ))}
            <button className="w-full text-left flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-acode-text-secondary hover:bg-acode-bg-hover transition-colors mt-1" onClick={() => { setShowAdd(true); setSelectedProvider(null); }}>
              <Plus className="w-4 h-4" />Add provider
            </button>
          </div>
        </div>
        <div className="flex-1 min-w-0 p-6">
          {showAdd ? <AddProviderForm onDone={() => setShowAdd(false)} /> : selected ? <ProviderDetail provider={selected} /> : <div className="flex items-center justify-center h-64 text-sm text-acode-text-muted">Select a provider to view details</div>}
        </div>
      </div>
    </>
  );
  ...
  return (
    <div>
      ...
      <div className="space-y-4 mb-6">
        <div>
          <label className="block text-sm text-acode-text-primary mb-1.5">Base URL</label>
        ...
        <div>
          <label className="block text-sm text-acode-text-primary mb-1.5">API key</label>
          <div className="flex gap-2">
            <div className="relative flex-1">
              ...
              <button className="absolute right-2.5 top-1/2 -translate-y-1/2 text-acode-text-muted hover:text-acode-text-primary transition-colors"
            ...
            <button className="px-4 py-1.5 bg-acode-bg-active hover:bg-acode-bg-tertiary text-sm text-acode-text-primary rounded-md border border-acode-border-primary transition-colors"
          ...
          {testStatus === "error" && (
            <p className="text-xs text-acode-git-deleted mt-1 break-all">{testError || "Connection failed. Check your API key and network."}</p>
          )}
          {testStatus === "ok" && (
            <p className="text-xs text-acode-git-added mt-1">Connected successfully.</p>
          )}
        </div>
      </div>
      ...
      {provider.models.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-3">
            <span className="text-sm font-medium text-acode-text-primary">Model list</span>
            <button onClick={() => setShowAddModel(true)} className="text-xs text-acode-accent-primary hover:text-acode-accent-hover transition-colors">+ Add model</button>
          </div>
...
function CustomProviderDetail({ provider }: { provider: ModelProvider }) {
  ...
  }, [provider.id, provider.baseUrl, provider.apiKey, provider.apiFormat, provider.name]);
  ...
  return (
    <div>
        ...
        <div className="flex items-center gap-3">
          {editingName ? (
            ...
            <>
              <span className="w-8 h-8 rounded-lg bg-acode-bg-active flex items-center justify-center text-sm font-bold text-acode-text-primary">{provider.name[0]}</span>
              <span className="text-lg font-semibold text-acode-text-primary">{provider.name}</span>
              <button onClick={() => setEditingName(true)} className="text-acode-text-muted hover:text-acode-text-primary transition-colors">
      ...
      <div className="space-y-4 mb-6">
        ...
        <div>
          <label className="block text-sm text-acode-text-primary mb-1.5">API format</label>
          <div className="relative">
      ...
      <div>
        <span className="text-sm font-medium text-acode-text-primary mb-3 block">Model list</span>
        {provider.models.length > 0 ? (
          <div className="space-y-2 mb-3">
            {provider.models.map((m) => (
              <div key={m.modelId} className="flex items-center gap-3 px-4 py-3 bg-acode-bg-tertiary border border-acode-border-primary rounded-xl">
                ...
                <button className="text-acode-text-muted hover:text-acode-text-primary transition-colors" title="Edit">
                ...
                <button className="text-acode-text-muted hover:text-acode-git-deleted transition-colors" title="Delete"
          ...
          <div className="text-center py-4 text-sm text-acode-text-muted mb-3">No models configured yet.</div>
        )}
        <button className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-acode-text-secondary hover:text-acode-text-primary bg-acode-bg-active hover:bg-acode-bg-tertiary rounded-lg border border-acode-border-primary transition-colors"
...
function AddModelModal({ providerId, onClose }: { providerId: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-center justify-center animate-fade-in" onClick={onClose}>
      <div className="w-[400px] bg-acode-bg-secondary border border-acode-border-primary rounded-xl shadow-2xl p-6" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-lg font-semibold text-acode-text-primary">Add model</h2>
          <button className="text-acode-text-muted hover:text-acode-text-primary transition-colors" onClick={onClose}>
            <X className="w-4 h-4" />
          </button>
        </div>
        <div className="space-y-4">
          <div>
            <label className="block text-sm text-acode-text-primary mb-1.5">Model ID</label>
            <input className="input-base w-full" value={modelId} placeholder="Model ID" onChange={(e) => setModelId(e.target.value)} autoFocus />
          </div>
          <div>
            <label className="block text-sm text-acode-text-primary mb-1.5">Context window</label>
            <input className="input-base w-full" type="number" value={contextWindow} onChange={(e) => setContextWindow(e.target.value)} />
          </div>
        </div>
        <div className="flex justify-end gap-2 mt-6">
          <button className="px-4 py-2 text-sm text-acode-text-secondary hover:text-acode-text-primary rounded-lg border border-acode-border-primary transition-colors" onClick={onClose}>Cancel</button>
          <button className="px-4 py-2 text-sm bg-acode-text-primary text-acode-bg-primary rounded-lg hover:opacity-90 transition-opacity"
...
function AddProviderForm({ onDone }: { onDone: () => void }) {
  ...
  return (
    <div>
      <h2 className="text-lg font-semibold text-acode-text-primary mb-1">Add model provider</h2>
      <p className="text-sm text-acode-text-muted mb-6">Configure a custom API endpoint and initial model.</p>
      <div className="space-y-5">
        <div><label className="block text-sm text-acode-text-primary mb-1.5">Name</label><input className="input-base w-full" placeholder="e.g. DeepSeek" value={name} onChange={(e) => setName(e.target.value)} /></div>
        <div><label className="block text-sm text-acode-text-primary mb-1.5">Base URL</label><input className="input-base w-full" placeholder="https://api.example.com/v1" value={baseUrl} onChange={(e) => setBaseUrl(e.target.value)} /></div>
        <div><label className="block text-sm text-acode-text-primary mb-1.5">API key</label><input className="input-base w-full" type="password" placeholder="Enter API key" value={apiKey} onChange={(e) => setApiKey(e.target.value)} /></div>
        <div><label className="block text-sm text-acode-text-primary mb-1.5">API format</label>
          ...
          </select><ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-4 h-4 text-acode-text-muted pointer-events-none" /></div>
        </div>
        <div>
          ...
          <div className="space-y-2 mb-2">
            {models.map((m, i) => (
              <div key={i} className="flex items-center gap-2">
                ...
                <button className="btn-icon text-acode-git-deleted" onClick={() => setModels(models.filter((_, j) => j !== i))}><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            ))}
          </div>
          <button className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-acode-text-secondary hover:text-acode-text-primary bg-acode-bg-active hover:bg-acode-bg-tertiary rounded-lg border border-acode-border-primary transition-colors" onClick={() => setModels([...models, { name: "", modelId: "", contextWindow: "" }])}><Plus className="w-3.5 h-3.5" />Add model</button>
        </div>
        <button className="px-4 py-2 bg-acode-accent-primary hover:bg-acode-accent-hover text-white text-sm rounded-lg transition-colors" onClick={() => {
...
function AgentsTab() {
  ...
  return (
    <>
          ...
          return (
            <button
              ...
              className={`w-full text-left surface p-4 transition-colors ${
                active ? "ring-1 ring-acode-accent-primary" : "hover:border-acode-accent-primary"
              }`}
            >
              <div className="flex items-start gap-3">
                <div className="w-10 h-10 rounded-lg bg-acode-bg-tertiary flex items-center justify-center flex-shrink-0">
                  <Icon className={`w-5 h-5 ${meta.color}`} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm font-semibold text-acode-text-primary">{meta.label}</span>
                    {agent.color && (
                      <span className="w-2 h-2 rounded-full" style={{ backgroundColor: agent.color }} />
                    )}
                    {active && <span className="text-[9px] uppercase tracking-wider text-acode-accent-primary font-medium">active</span>}
                  </div>
      ...
      >
      <div className="border border-acode-border-primary rounded-lg overflow-hidden mb-8">
        {subagents.map((agent: AgentInfo, idx: number) => (
          <div
            key={agent.name}
            className={`flex items-start gap-3 px-3 py-2.5 hover:bg-acode-bg-hover transition-colors ${
              idx < subagents.length - 1 ? "border-b border-acode-border-primary" : ""
            }`}
          >
            <div className="w-8 h-8 rounded-md bg-acode-bg-tertiary flex items-center justify-center flex-shrink-0 mt-0.5">
              <Bot className="w-4 h-4 text-acode-text-muted" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="text-sm text-acode-text-primary font-medium">{agent.name}</span>
                ...
                {agent.native && <span className="chip text-acode-accent-primary">built-in</span>}
              </div>
              <div className="text-xs text-acode-text-muted mt-0.5">{agent.description}</div>
              <div className="mt-1 text-[10px] text-acode-text-muted font-mono">
...
const ACTION_META: Record<PermissionAction, { label: string; color: string; desc: string }> = {
  allow: { label: "Allow", color: "text-acode-git-added", desc: "Run without asking" },
  ask: { label: "Ask", color: "text-amber-400", desc: "Prompt for confirmation" },
  deny: { label: "Deny", color: "text-acode-git-deleted", desc: "Block the action" },
};
...
function PermissionsTab() {
  ...
  return (
    <>
      ...
      >
        <div className="flex items-center gap-2">
          <span className="text-xs text-acode-text-muted">Active agent:</span>
          <select
      ...
      >
        <div className="surface p-3">
          <div className="grid grid-cols-[180px_1fr_120px_auto] gap-2 items-end">
            <div>
              <label className="text-[10px] uppercase tracking-wider text-acode-text-muted block mb-1">Permission</label>
            ...
            <div>
              <label className="text-[10px] uppercase tracking-wider text-acode-text-muted block mb-1">Pattern</label>
              <input
            ...
            <div>
              <label className="text-[10px] uppercase tracking-wider text-acode-text-muted block mb-1">Action</label>
            ...
            <button
              className="px-3 py-1.5 bg-acode-accent-primary hover:bg-acode-accent-hover text-white text-xs rounded-md transition-colors"
              onClick={() => {
      ...
      <Section title="Agent defaults" hint="These rules ship with the agent. Add your own rules above to override them.">
        <div className="border border-acode-border-primary rounded-lg overflow-hidden">
          <div className="grid grid-cols-[1fr_1fr_80px_24px] px-3 py-2 bg-acode-bg-tertiary text-[10px] uppercase tracking-wider text-acode-text-muted">
          ...
          {allRules.length === 0 && (
            <div className="px-3 py-4 text-center text-xs text-acode-text-muted">No rules.</div>
          )}
          {allRules.map((r: PermissionRule, idx: number) => {
            ...
            return (
              <div
                ...
                className="grid grid-cols-[1fr_1fr_80px_24px] px-3 py-2 items-center text-xs border-t border-acode-border-primary hover:bg-acode-bg-hover/40"
              >
                <span className="text-acode-text-primary font-mono">{r.permission}</span>
                <span className="text-acode-text-secondary font-mono truncate">{r.pattern}</span>
      ...
      {userRules.length > 0 && (
        <Section title="Your custom rules" hint="These override the agent defaults. Higher entries win over lower ones.">
          ...
          <button
            className="mt-2 px-3 py-1.5 text-xs text-acode-text-secondary hover:text-acode-text-primary bg-acode-bg-active hover:bg-acode-bg-tertiary rounded-md border border-acode-border-primary transition-colors"
      ...
      <Section title="How permissions are evaluated" hint="For each tool call, ACode walks the rules top-to-bottom and uses the first match.">
        <ol className="text-xs text-acode-text-secondary space-y-1.5 list-decimal pl-5">
          <li>Exact <code className="text-mono text-acode-text-primary">(permission, pattern)</code> match wins.</li>
          <li>Then the wildcard pattern <code className="text-mono text-acode-text-primary">*</code> for that permission.</li>
          <li>Then the global wildcard <code className="text-mono text-acode-text-primary">*</code> rule.</li>
          <li>If nothing matches, the action is <strong>ask</strong>.</li>
        </ol>
        <p className="text-xs text-acode-text-muted mt-3">
  ...
  return (
    <>
      <div className="flex items-center gap-3 mb-2">
        <h1 className="text-3xl font-bold text-acode-text-primary">Skills</h1>
        <span className="text-xs text-acode-text-muted">{skills.length}</span>
      </div>
      <p className="text-sm text-acode-text-muted mb-6">Manage workspace and user skills. Enabled skills can be referenced in chat with <code className="px-1 py-0.5 rounded bg-acode-bg-tertiary font-mono text-acode-text-primary">$skill-name</code>.</p>
      ...
      <div className="flex items-center gap-2 mb-3">
        ...
        />
        <button onClick={() => setShowAdd((v) => !v)} className="w-8 h-8 flex items-center justify-center rounded-md bg-acode-bg-tertiary border border-acode-border-primary text-acode-text-secondary hover:bg-acode-bg-hover hover:text-acode-text-primary transition-colors" title="Add skill">
          <Plus className="w-3.5 h-3.5" />
        </button>
        <button className="w-8 h-8 flex items-center justify-center rounded-md bg-acode-bg-tertiary border border-acode-border-primary text-acode-text-secondary hover:bg-acode-bg-hover hover:text-acode-text-primary transition-colors" title="Refresh">
      ...
      {/* Add skill form */}
      {showAdd && (
        <div className="mb-4 bg-acode-bg-secondary border border-acode-border-primary rounded-xl p-4">
          <div className="text-sm font-medium text-acode-text-primary mb-3">New skill</div>
          <div className="grid grid-cols-2 gap-3 mb-3">
            <div>
              <label className="block text-xs text-acode-text-muted mb-1">Name</label>
              <input className="input-base text-sm w-full" placeholder="my-skill" value={newName} onChange={(e) => setNewName(e.target.value)} />
              <div className="text-[10px] text-acode-text-muted mt-1">Used as $my-skill in chat</div>
            </div>
            <div>
              <label className="block text-xs text-acode-text-muted mb-1">Scope</label>
              <div className="relative">
                ...
                <ChevronDown className="absolute right-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-acode-text-muted pointer-events-none" />
              </div>
            </div>
          </div>
          <div className="mb-3">
            <label className="block text-xs text-acode-text-muted mb-1">Description</label>
          ...
          <div className="mb-3">
            <label className="block text-xs text-acode-text-muted mb-1">Prompt (instructions for the agent)</label>
          ...
          <div className="flex items-center justify-end gap-2">
            <button onClick={() => setShowAdd(false)} className="px-3 py-1.5 text-xs rounded-md text-acode-text-secondary hover:bg-acode-bg-hover transition-colors">Cancel</button>
            <button onClick={onAdd} className="px-3 py-1.5 text-xs rounded-md bg-acode-accent-primary hover:bg-acode-accent-hover text-white transition-colors">Add skill</button>
          </div>
        </div>
      )}

      <div className="text-[11px] uppercase tracking-wider text-acode-text-muted px-2 mb-2 flex items-center gap-2">
        <span>Workspace and personal skills</span>
        <span className="text-acode-text-muted/60">· {filtered.length} {filtered.length === 1 ? "skill" : "skills"}</span>
      </div>
      <div className="space-y-1">
        {filtered.length === 0 && (
          <div className="text-center text-sm text-acode-text-muted py-8">No skills match "{query}".</div>
        )}
        {filtered.map((s) => (
          <div key={s.name} className="group flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-acode-bg-hover transition-colors">
            <div className="flex-shrink-0 w-8 h-8 rounded-md bg-acode-bg-tertiary flex items-center justify-center">
              <Sparkles className="w-3.5 h-3.5 text-acode-accent-primary" />
            </div>
            <div className="flex-1 min-w-0">
              <div className="text-sm text-acode-text-primary font-medium truncate">${s.name}</div>
              <div className="text-xs text-acode-text-muted line-clamp-1">{s.description}</div>
            </div>
            ...
            <button
              ...
              className="opacity-0 group-hover:opacity-100 p-1.5 rounded-md text-acode-text-muted hover:text-acode-git-deleted hover:bg-acode-bg-active transition-all"
              title="Remove skill"
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-2">
        <h1 className="text-3xl font-bold text-acode-text-primary">MCP Servers</h1>
        {view === "list" ? (
          <button onClick={() => setView("add")} className="flex items-center gap-1.5 px-3 py-1.5 text-xs rounded-md bg-acode-accent-primary hover:bg-acode-accent-hover text-white transition-colors">
            <Plus className="w-3.5 h-3.5" /> Add server
          </button>
        ) : (
          <button onClick={() => { reset(); setView("list"); }} className="flex items-center gap-1 text-sm text-acode-text-secondary hover:text-acode-text-primary">
      ...
      {view === "list" ? (
        <>
          <p className="text-sm text-acode-text-muted mb-6">Connect MCP servers to extend the agent with new tools. Use the toggle to connect / disconnect.</p>
          <div className="space-y-1">
            {mcpServers.length === 0 && (
              <div className="text-center text-sm text-acode-text-muted py-8">No MCP servers configured.</div>
            )}
            {mcpServers.map((m) => (
              <div key={m.name} className="group flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-acode-bg-hover transition-colors">
                <div className="flex-shrink-0 w-8 h-8 rounded-md bg-acode-bg-tertiary flex items-center justify-center">
                  <Plug className="w-3.5 h-3.5 text-acode-text-muted" />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-acode-text-primary font-medium">{m.name}</div>
                  <div className="text-xs text-acode-text-muted truncate">
                    {m.transport === "http" ? (m.url ?? "—") : `${m.command ?? "—"} ${(m.args ?? []).join(" ")}`.trim()}
                    {m.tools && ` · ${m.tools.length} tool${m.tools.length === 1 ? "" : "s"}`}
                  </div>
                </div>
                <span className={`chip flex-shrink-0 ${m.status === "connected" ? "text-acode-git-added" : m.status === "error" ? "text-acode-git-deleted" : "text-acode-text-muted"}`}>
                  {m.status}
                </span>
                <button
        ...
        <>
          <h2 className="text-xl font-semibold text-acode-text-primary mb-1">New MCP server</h2>
          <p className="text-sm text-acode-text-muted mb-4">Fill in the new MCP configuration, then save to return to the list.</p>

          <div className="flex items-center gap-1 bg-acode-bg-tertiary border border-acode-border-primary rounded-md p-0.5 w-fit mb-5">
            <button onClick={() => setEditMode("form")} className={`px-3 py-1 text-xs rounded transition-colors ${editMode === "form" ? "bg-acode-bg-active text-acode-text-primary font-medium" : "text-acode-text-secondary hover:text-acode-text-primary"}`}>Form</button>
            <button onClick={() => setEditMode("json")} className={`px-3 py-1 text-xs rounded transition-colors ${editMode === "json" ? "bg-acode-bg-active text-acode-text-primary font-medium" : "text-acode-text-secondary hover:text-acode-text-primary"}`}>JSON</button>
          </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm text-acode-text-primary mb-1.5">Name</label>
                <input className="input-base text-sm w-full" placeholder="my-mcp-server" value={name} onChange={(e) => setName(e.target.value)} />
              </div>
              <div>
                <label className="block text-sm text-acode-text-primary mb-1.5">Scope</label>
                <div className="relative">
            ...
            <div>
              <label className="block text-sm text-acode-text-primary mb-1.5">Type</label>
              <div className="relative">
            ...
            {transport === "stdio" ? (
              <>
                <div>
                  <label className="block text-sm text-acode-text-primary mb-1.5">Command</label>
                  <input className="input-base text-sm w-full" placeholder="npx" value={command} onChange={(e) => setCommand(e.target.value)} />
                </div>
                <div>
                  <label className="block text-sm text-acode-text-primary mb-1.5">Arguments (space separated)</label>
              ...
              <div>
                <label className="block text-sm text-acode-text-primary mb-1.5">Endpoint URL</label>
            ...
            <div>
              <button onClick={() => setEnvOpen((v) => !v)} className="flex items-center gap-1.5 text-sm text-acode-text-primary">
                <ChevronDown className={`w-3.5 h-3.5 transition-transform ${envOpen ? "" : "-rotate-90"}`} />
                Environment variables <span className="text-acode-text-muted">(optional)</span>
              </button>
              {envOpen && (
                <div className="mt-2 space-y-1.5">
                  {envEntries.map((e, i) => (
                    <div key={i} className="flex items-center gap-2">
                      ...
                      <button onClick={() => setEnvEntries((arr) => arr.filter((_, j) => j !== i))} className="p-1.5 rounded text-acode-text-muted hover:text-acode-git-deleted">
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </div>
                  ))}
                  <button onClick={() => setEnvEntries((arr) => [...arr, { key: "", value: "" }])} className="text-xs text-acode-text-secondary hover:text-acode-text-primary">+ Add variable</button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-end gap-2 mt-6">
            <button onClick={() => { reset(); setView("list"); }} className="px-3 py-1.5 text-sm rounded-md text-acode-text-secondary hover:bg-acode-bg-hover transition-colors">Cancel</button>
            {editMode === "form" ? (
...
function PluginsTab() {
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-acode-text-primary mb-2">Plugins</h1>
      <p className="text-sm text-acode-text-muted mb-8">Extend ACode with community plugins. Plugins run in a sandboxed renderer context.</p>
      <Card title="Installed plugins" description="Toggle each plugin to enable or disable it for the current workspace.">
        <div className="divide-y divide-acode-border-primary">
          {installed.length === 0 && (
            <div className="text-center text-sm text-acode-text-muted py-6">No plugins installed.</div>
          )}
          {installed.map((p) => (
            <div key={p.id} className="flex items-center justify-between gap-3 py-3 first:pt-0 last:pb-0">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  <span className="text-sm text-acode-text-primary font-medium truncate">{p.name}</span>
                  <span className="text-[10px] text-acode-text-muted">v{p.version}</span>
                </div>
                <div className="text-xs text-acode-text-muted line-clamp-1">{p.description}</div>
              </div>
              <button
                onClick={() => onUninstall(p.id, p.name)}
                disabled={uninstalling === p.id}
                className="p-1.5 rounded-md text-acode-text-muted hover:text-acode-git-deleted hover:bg-acode-bg-active transition-colors disabled:opacity-50"
                title="Uninstall"
      ...
      <Card title="Install from marketplace" description="Browse and install plugins from the ACode marketplace.">
        <div className="flex items-center gap-2">
          <button className="px-4 py-2 bg-acode-accent-primary hover:bg-acode-accent-hover text-white text-sm rounded-md transition-colors"
            onClick={() => toast({ kind: "info", title: "Marketplace coming soon" })}>
            Open marketplace
          </button>
          <button className="px-4 py-2 bg-acode-bg-tertiary hover:bg-acode-bg-hover text-sm text-acode-text-primary rounded-md border border-acode-border-primary transition-colors"
...
function CommandsTab() {
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-acode-text-primary mb-2">Commands</h1>
      <p className="text-sm text-acode-text-muted mb-8">Customize keyboard shortcuts for built-in and plugin commands.</p>
      <Card title="Keyboard shortcuts" description="Click any shortcut to record a new binding.">
        <div className="divide-y divide-acode-border-primary">
          {commands.length === 0 && (
            <div className="text-center text-sm text-acode-text-muted py-6">Custom keybindings coming soon.</div>
          )}
          {commands.map((c) => (
            <div key={c.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
              <div className="min-w-0">
                <div className="text-sm text-acode-text-primary font-medium">{c.name}</div>
                <div className="text-xs text-acode-text-muted">{c.description}</div>
              </div>
              <button
                onClick={() => setEditing(c.id)}
                className="px-2.5 py-1 text-xs font-mono rounded-md bg-acode-bg-tertiary border border-acode-border-primary hover:bg-acode-bg-hover transition-colors"
              >
      ...
      <Card title="Custom commands" description="Create your own reusable commands that run agent tasks or scripts.">
        <button className="px-4 py-2 bg-acode-bg-active hover:bg-acode-bg-tertiary text-sm text-acode-text-primary rounded-md border border-acode-border-primary transition-colors">
          + New command
        </button>
      </Card>
    </>
...
function IndexingTab() {
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-acode-text-primary mb-2">Indexing</h1>
      <p className="text-sm text-acode-text-muted mb-8">Configure how ACode indexes your workspace for fast search and code understanding.</p>
      ...
      <Card title="Max file size" description="Skip files larger than this size when indexing (in MB).">
        <div className="flex items-center gap-4">
          <input type="range" min={1} max={50} value={maxFileSize} onChange={(e) => setMaxFileSize(Number(e.target.value))} className="flex-1 accent-acode-accent-primary" />
          <span className="text-sm text-acode-text-primary w-12 text-right">{maxFileSize} MB</span>
        </div>
      </Card>
      <Card title="Excluded patterns" description="Glob patterns to skip during indexing, one per line.">
        ...
        />
        <button className="mt-3 px-4 py-1.5 bg-acode-bg-active hover:bg-acode-bg-tertiary text-sm text-acode-text-primary rounded-md border border-acode-border-primary transition-colors" onClick={saveIndexing}>
          Save patterns
        </button>
      </Card>
    </>
...
function OnboardTab() {
  ...
  return (
    <>
      <h1 className="text-3xl font-bold text-acode-text-primary mb-2">Onboard</h1>
      <p className="text-sm text-acode-text-muted mb-8">Replay the onboarding tour or jump to any step.</p>
      <Card title={`Step ${step + 1} of ${steps.length}: ${current.title}`} description={current.body}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1">
            {steps.map((_, i) => (
              <div key={i} className={`h-1 rounded-full transition-all ${i === step ? "w-6 bg-acode-accent-primary" : "w-1.5 bg-acode-bg-tertiary"}`} />
            ))}
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => setStep(Math.max(0, step - 1))} disabled={step === 0} className="px-3 py-1.5 text-sm rounded-md text-acode-text-secondary hover:bg-acode-bg-hover disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
              Back
            </button>
            <button onClick={() => setStep(Math.min(steps.length - 1, step + 1))} disabled={step === steps.length - 1} className="px-3 py-1.5 text-sm rounded-md bg-acode-accent-primary hover:bg-acode-accent-hover text-white disabled:opacity-30 disabled:cursor-not-allowed transition-colors">
              Next
            </button>
          </div>
        </div>
...
function Toggle({ checked, onChange, label }: { checked: boolean; onChange: () => void; label?: string }) {
  return (
    <button
      ...
      className={`relative inline-flex h-6 w-11 rounded-full transition-colors flex-shrink-0 ${checked ? "bg-acode-accent-primary" : "bg-acode-bg-active"}`}
      onClick={onChange}
    >
