...
const BASH_ARITY: Record<string, number> = {
  ...
  "firebase deploy": 2, "flyctl deploy": 2, "git checkout": 2, "go build": 2,
  ...
  "ng generate": 2, "npm install": 2, "nvm use": 2, "nx build": 2,
  "openssl genrsa": 2, "pip install": 2, "pipenv install": 2, "pnpm install": 2,
  "podman run": 2, "psql -d": 2, "pulumi up": 2, "pyenv install": 2,
...
const BUILD_AGENT: AgentInfo = {
  ...
  permission: mergeRulesets(
    DEFAULT_PERMISSIONS,
    fromConfig({ question: "allow", plan_enter: "allow" })
  ),
};
