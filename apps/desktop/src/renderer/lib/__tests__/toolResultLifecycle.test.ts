...
function resetStore() {
  useChat.setState({
    ...
    runtimeState: {
      ...
      toolCallStatuses: new Map<string, import("../../lib/agentRuntimeContract").ToolCallStatus>(),
...
describe("basic message flow", () => {
  ...
  it("message-delta appends content to streamingContent", () => {
    setupSession();
    useChat.getState().appendStream({ type: "message-start", messageId: "msg-1" });
    useChat.getState().appendStream({ type: "message-delta", messageId: "msg-1", content: "Hello" });
    useChat.getState().appendStream({ type: "message-delta", messageId: "msg-1", content: " World" });
  ...
  it("message-delta trims content beyond 200K chars", () => {
    ...
    useChat.getState().appendStream({ type: "message-delta", messageId: "msg-1", content: largeChunk });
    // Second chunk pushes total past 200K
    useChat.getState().appendStream({ type: "message-delta", messageId: "msg-1", content: largeChunk });

    expect(useChat.getState().streamingContent.length).toBeLessThanOrEqual(200000);
  });
  ...
  it("message-end creates final assistant message and clears streaming state", () => {
    ...
    useChat.getState().appendStream({ type: "message-start", messageId: "msg-1" });
    useChat.getState().appendStream({ type: "message-delta", messageId: "msg-1", content: "Hello world" });
    useChat.getState().appendStream({ type: "message-end", messageId: "msg-1" });
...
describe("tool call lifecycle", () => {
  ...
  it("tool-call after message-delta appends to pendingToolCalls (streaming continues)", () => {
    setupSession();
    useChat.getState().appendStream({ type: "message-start", messageId: "msg-1" });
    useChat.getState().appendStream({ type: "message-delta", messageId: "msg-1", content: "Let me check" });

    const tcEvent: StreamEvent = {
      type: "tool-call",
      toolCall: { id: "tc-3", name: "bash", args: { command: "ls" }, status: "pending" },
    };
    useChat.getState().appendStream(tcEvent);
...
describe("diff-proposed binding", () => {
  ...
  function makePendingTc(id: string, name: string, path: string, status: ToolCall["status"] = "awaiting-approval"): ToolCall {
    return {
      id,
      name,
      args: { path },
  ...
  it("binds diff to tool call via Strategy 2 (filePath + edit tool name)", () => {
    ...
    useChat.setState({
      pendingToolCalls: [makePendingTc("tc-write", "write_file", "/src/file.ts")],
    });
  ...
  it("binds diff to tool call via Strategy 3 (content hash matching for write_file)", () => {
    ...
    const diffEvent: StreamEvent = {
      ...
      proposal: {
        ...
        hunks: [{ oldStart: 0, oldLines: 0, newStart: 1, newLines: 1, lines: [{ type: "add", content: "new content here", newLineNumber: 1 }] }],
        createdAt: Date.now(),
      },
    };
    useChat.getState().appendStream(diffEvent);

    const boundTc = useChat.getState().pendingToolCalls.find(tc => tc.id === "tc-write-2");
    expect(boundTc).toBeDefined();
  ...
  it("binds diff to tool call via Strategy 4 (most recent pending edit with same filePath)", () => {
    ...
    const diffEvent: StreamEvent = {
      ...
      proposal: {
        ...
        oldContent: "a",
        newContent: "b",
        hunks: [{ oldStart: 1, oldLines: 1, newStart: 1, newLines: 1, lines: [{ type: "remove", content: "a", oldLineNumber: 1 }, { type: "add", content: "b", newLineNumber: 1 }] }],
        createdAt: Date.now(),
      },
    };
    useChat.getState().appendStream(diffEvent);

    const boundTc = useChat.getState().pendingToolCalls.find(tc => tc.id === "tc-recent");
    expect(boundTc).toBeDefined();
    expect(boundTc!.diffId).toBe("diff-3");
    // The other tool call should not be bound
    const otherTc = useChat.getState().pendingToolCalls.find(tc => tc.id === "tc-old");
...
describe("thinking content handling", () => {
  ...
  it("thinking event sets thinkingContent", () => {
    ...
    expect(useChat.getState().thinkingContent).toBe("Let me think about this carefully...");
  });

  it("multiple thinking events accumulate", () => {
    setupSession();
    useChat.getState().appendStream({ type: "message-start", messageId: "msg-1" });
    useChat.getState().appendStream({ type: "thinking", messageId: "msg-1", content: "First thought. " });
    useChat.getState().appendStream({ type: "thinking", messageId: "msg-1", content: "Second thought." });

    expect(useChat.getState().thinkingContent).toBe("First thought. Second thought.");
  });
});
...
describe("error event handling", () => {
  ...
  it("error event resets streaming state and adds error message", () => {
    ...
    const errEvent: StreamEvent = { type: "error", error: "API connection failed" };
    ...
    const errMsg = state.messages.find(m => m.content.includes("API connection failed"));
  ...
  it("error event during streaming appends to existing messages", () => {
    ...
    useChat.getState().appendStream({ type: "message-start", messageId: "msg-1" });
    useChat.getState().appendStream({ type: "error", error: "Rate limit exceeded" });
